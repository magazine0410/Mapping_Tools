...
namespace Mapping_Tools.Classes.Tools {
    ...
    public class RhythmGuide {
        ...
        public class RhythmGuideGeneratorArgs : BindableBase {
            ...
            private string exportPath = Path.Combine(CorePlatform.Paths.ExportPath, @"rhythm_guide.osu");
        ...
        public static void GenerateRhythmGuide(RhythmGuideGeneratorArgs args) {
            ...
            switch (args.ExportMode) {
                case ExportMode.NewMap:
                    var beatmap = MergeBeatmaps(args.Paths.Select(o => CorePlatform.EditorReader.GetNewestVersionOrNot(o, reader).Beatmap).ToArray(),
                        args);

                    var editor = new Editor {TextFile = beatmap, Path = args.ExportPath};
                    editor.SaveFile();
                    CorePlatform.Shell.OpenFolder(Path.GetDirectoryName(args.ExportPath) ??
                                                  throw new ArgumentException("Export path must be a file."));
                    break;
                case ExportMode.AddToMap:
                    var editor2 = CorePlatform.EditorReader.GetNewestVersionOrNot(args.ExportPath, reader);

                    PopulateBeatmap(editor2.Beatmap,
                        args.Paths.Select(o => CorePlatform.EditorReader.GetNewestVersionOrNot(o, reader).Beatmap),
                        args);
